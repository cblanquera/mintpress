...
describe('Mintpress Tests', function () {
  ...
  it('Should stress batch mint', async function () {
    ...
    expect(await contractOwner.withContract.classURI(classId1)).to.equal(classURI1)

    const classId2 = 200
    const classSize2 = 1
    const classURI2 = 'ipfs://abc234'
    await contractOwner.withContract.register(classId2, classSize2, classURI2)
    expect(await contractOwner.withContract.classURI(classId2)).to.equal(classURI2)

    const classId3 = 300
    const classSize3 = 0
    const classURI3 = 'ipfs://abc345'
    await contractOwner.withContract.register(classId3, classSize3, classURI3)
    expect(await contractOwner.withContract.classURI(classId3)).to.equal(classURI3)

    //----------------------------------------//
    // This is the minting
    const fromTokenId = 2000
    //fast forward ... (go straight to the token owner)
    await contractOwner.withContract.batchMint(
      [classId1, classId1, classId2, classId3], 
      fromTokenId, 
      tokenOwner.address
    )
    expect(await contractOwner.withContract.ownerOf(fromTokenId)).to.equal(tokenOwner.address)
    expect(await contractOwner.withContract.ownerOf(fromTokenId + 1)).to.equal(tokenOwner.address)
    expect(await contractOwner.withContract.ownerOf(fromTokenId + 2)).to.equal(tokenOwner.address)
    expect(await contractOwner.withContract.ownerOf(fromTokenId + 3)).to.equal(tokenOwner.address)
    expect(await contractOwner.withContract.classOf(fromTokenId)).to.equal(classId1)
    expect(await contractOwner.withContract.classOf(fromTokenId + 1)).to.equal(classId1)
    expect(await contractOwner.withContract.classOf(fromTokenId + 2)).to.equal(classId2)
    expect(await contractOwner.withContract.classOf(fromTokenId + 3)).to.equal(classId3)
    expect(await contractOwner.withContract.tokenURI(fromTokenId)).to.equal(classURI1)
    expect(await contractOwner.withContract.tokenURI(fromTokenId + 1)).to.equal(classURI1)
    expect(await contractOwner.withContract.tokenURI(fromTokenId + 2)).to.equal(classURI2)
    expect(await contractOwner.withContract.tokenURI(fromTokenId + 3)).to.equal(classURI3)

    //----------------------------------------//
    // This is the test
    expect(
      contractOwner.withContract.batchMint(
        [classId1, classId1, classId2, classId3], 
        fromTokenId, 
        tokenOwner.address
      )
    ...
    await contractOwner.withContract.batchClassMint(
      3, 
      5000,
      500000, 
      tokenOwner.address
